...
<div id="dashboard-widgets" class="metabox-holder<?php echo $columns_css; ?>">
 ...
 $comment_row_class = ''; if ( get_option( 'show_avatars' ) ) { echo get_avatar( $comment, 50, 'mystery' ); $comment_row_class .= ' has-avatar'; } ?>
			...
			<p class="comment-meta">
				<?php
 if ( $comment_post_link ) { printf( __( 'From %1$s on %2$s %3$s' ), '<cite class="comment-author">' . get_comment_author_link( $comment ) . '</cite>', $comment_post_link, '<span class="approve">' . __( '[Pending]' ) . '</span>' ); } else { printf( __( 'From %1$s %2$s' ), '<cite class="comment-author">' . get_comment_author_link( $comment ) . '</cite>', '<span class="approve">' . __( '[Pending]' ) . '</span>' ); } ?>
			</p>

				<?php
 else : switch ( $comment->comment_type ) { case 'pingback': $type = __( 'Pingback' ); break; case 'trackback': $type = __( 'Trackback' ); break; default: $type = ucwords( $comment->comment_type ); } $type = esc_html( $type ); ?>
			<div class="dashboard-comment-wrap has-row-actions">
			<p class="comment-meta">
				<?php
 if ( $comment_post_link ) { printf( _x( '%1$s on %2$s %3$s', 'dashboard' ), "<strong>$type</strong>", $comment_post_link, '<span class="approve">' . __( '[Pending]' ) . '</span>' ); } else { printf( _x( '%1$s %2$s', 'dashboard' ), "<strong>$type</strong>", '<span class="approve">' . __( '[Pending]' ) . '</span>' ); } ?>
			</p>
 ...
 printf( '<a href="%1$s" target="_blank">%2$s <span class="screen-reader-text"> %3$s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a>', esc_url( _x( 'https://wordpress.org/news/', 'Events and News dashboard widget' ) ), __( 'News' ), __( '(opens in a new tab)' ) ); ?>
	...
	<?php
} function wp_print_community_events_markup() { $community_events_notice = '<p class="hide-if-js">' . ( 'This widget requires JavaScript.' ) . '</p>'; $community_events_notice .= '<p class="community-events-error-occurred" aria-hidden="true">' . __( 'An error occurred. Please try again.' ) . '</p>'; $community_events_notice .= '<p class="community-events-could-not-locate" aria-hidden="true"></p>'; wp_admin_notice( $community_events_notice, array( 'type' => 'error', 'additional_classes' => array( 'community-events-errors', 'inline', 'hide-if-js' ), 'paragraph_wrap' => false, ) ); ?>
 ...
 printf( __( 'Attend an upcoming event near %s.' ), '<strong>{{ data.location.description }}</strong>' ); ?>
	...
	<script id="tmpl-community-events-could-not-locate" type="text/template">
		<?php
 printf( __( '%s could not be located. Please try another nearby city. For example: Kansas City; Springfield; Portland.' ), '<em>{{data.unknownCity}}</em>' ); ?>
	</script>
 ...
 $text = sprintf( __( '%1$s MB (%2$s%%) Space Used' ), number_format_i18n( $used, 2 ), $percentused ); printf( '<a href="%1$s" class="musublink">%2$s<span class="screen-reader-text"> (%3$s)</span></a>', esc_url( admin_url( 'upload.php' ) ), $text, __( 'Manage Uploads' ) ); ?>
	...
	<p>
		...
		<?php
 if ( ! empty( $response['recommended_version'] ) ) { printf( __( 'The minimum recommended version of PHP is %s.' ), $response['recommended_version'] ); } ?>
	</p>
 ...
 printf( _n( 'Take a look at the <strong>%1$d item</strong> on the <a href="%2$s">Site Health screen</a>.', 'Take a look at the <strong>%1$d items</strong> on the <a href="%2$s">Site Health screen</a>.', $issues_total ), $issues_total, esc_url( admin_url( 'site-health.php' ) ) ); ?>
	...
	<?php
} function wp_dashboard_empty() {} function wp_welcome_panel() { list( $display_version ) = explode( '-', get_bloginfo( 'version' ) ); $can_customize = current_user_can( 'customize' ); $is_block_theme = wp_is_block_theme(); ?>
