<?php
 ...
 printf( __( 'Add widget: %s' ), $widget_title ); ?>
	...
	<?php
 if ( isset( $control['callback'] ) ) { $has_form = call_user_func_array( $control['callback'], $control['params'] ); } else { echo "\t\t<p>" . __( 'There are no options for this widget.' ) . "</p>\n"; } $noform_class = ''; if ( 'noform' === $has_form ) { $noform_class = ' widget-control-noform'; } ?>
	...
	<?php
 $widget_description = wp_widget_description( $widget_id ); echo ( $widget_description ) ? "$widget_description\n" : "$widget_title\n"; ?>
	</div>
	<?php
 echo $sidebar_args['after_widget']; return $sidebar_args; } function wp_widgets_access_body_class( $classes ) { return "$classes widgets_access "; } 