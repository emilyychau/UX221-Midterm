...
<?php
if ( ! empty( $tabs['upload'] ) && current_user_can( 'upload_plugins' ) ) { printf( ' <a href="%s" class="upload-view-toggle page-title-action"><span class="upload">%s</span><span class="browse">%s</span></a>', ( 'upload' === $tab ) ? self_admin_url( 'plugin-install.php' ) : self_admin_url( 'plugin-install.php?tab=upload' ), __( 'Upload Plugin' ), __( 'Browse Plugins' ) ); } ?>
...
<?php
 ...
 do_action( 'install_plugins_upload' ); ?>
	...
	<?php
 $wp_list_table->views(); } do_action( "install_plugins_{$tab}", $paged ); ?>
