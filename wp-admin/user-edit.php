<?php
 ...
 do_action( 'user_edit_form_tag' ); ?>
				...
				<table class="form-table" role="presentation">
					...
					<?php
 $show_syntax_highlighting_preference = ( user_can( $profile_user, 'edit_theme_options' ) || user_can( $profile_user, 'edit_plugins' ) || user_can( $profile_user, 'edit_themes' ) ); ?>
 ...
 do_action( 'admin_color_scheme_picker', $user_id ); ?>
					...
					<?php
 $languages = get_available_languages(); $can_install_translations = current_user_can( 'install_languages' ) && wp_can_install_language_pack(); ?>
					...
					<tr class="user-language-wrap">
						...
						<td>
							<?php
 $user_locale = $profile_user->locale; if ( 'en_US' === $user_locale ) { $user_locale = ''; } elseif ( '' === $user_locale || ! in_array( $user_locale, $languages, true ) ) { $user_locale = 'site-default'; } wp_dropdown_languages( array( 'name' => 'locale', 'id' => 'locale', 'selected' => $user_locale, 'languages' => $languages, 'show_available_translations' => $can_install_translations, 'show_option_site_default' => true, ) ); ?>
						</td>
					</tr>
					<?php endif; ?>
 ...
 do_action( 'personal_options', $profile_user ); ?>
				...
				<?php
 if ( IS_PROFILE_PAGE ) { do_action( 'profile_personal_options', $profile_user ); } ?>
						...
						<tr class="user-role-wrap">
							...
							<td>
								<select name="role" id="role">
									<?php
 $user_roles = array_intersect( array_values( $profile_user->roles ), array_keys( get_editable_roles() ) ); $user_role = reset( $user_roles ); wp_dropdown_roles( $user_role ); if ( $user_role ) { echo '<option value="">' . __( '&mdash; No role for this site &mdash;' ) . '</option>'; } else { echo '<option value="" selected="selected">' . __( '&mdash; No role for this site &mdash;' ) . '</option>'; } ?>
							</select>
							</td>
						</tr>
					<?php endif; ?>
				...
				<table class="form-table" role="presentation">
					<tr class="user-email-wrap">
						...
						<td>
							...
							<?php
 $new_email = get_user_meta( $current_user->ID, '_new_email', true ); if ( $new_email && $new_email['newemail'] !== $current_user->user_email && $profile_user->ID === $current_user->ID ) : $pending_change_message = sprintf( __( 'There is a pending change of your email to %s.' ), '<code>' . esc_html( $new_email['newemail'] ) . '</code>' ); $pending_change_message .= sprintf( ' <a href="%1$s">%2$s</a>', esc_url( wp_nonce_url( self_admin_url( 'profile.php?dismiss=' . $current_user->ID . '_new_email' ), 'dismiss-' . $current_user->ID . '_new_email' ) ), __( 'Cancel' ) ); wp_admin_notice( $pending_change_message, array( 'additional_classes' => array( 'updated', 'inline' ), ) ); endif; ?>
						</td>
					</tr>
 ...
 echo apply_filters( "user_{$name}_label", $desc ); ?>
				...
				<table class="form-table" role="presentation">
					<tr class="user-description-wrap">
						...
						<td><textarea name="description" id="description" rows="5" cols="30"><?php echo $profile_user->description; ?></textarea>
					...
					<?php if ( get_option( 'show_avatars' ) ) : ?>
						<tr class="user-profile-picture">
							...
							<td>
								...
								<p class="description">
									<?php
 if ( IS_PROFILE_PAGE ) { $description = sprintf( __( '<a href="%s">You can change your profile picture on Gravatar</a>.' ), __( 'https://en.gravatar.com/' ) ); } else { $description = ''; } echo apply_filters( 'user_profile_picture_description', $description, $profile_user ); ?>
								</p>
							</td>
						</tr>
					<?php endif; ?>
							...
							<?php if ( ! IS_PROFILE_PAGE && true === wp_is_password_reset_allowed_for_user( $profile_user ) ) : ?>
								<tr class="user-generate-reset-link-wrap hide-if-no-js">
									...
									<td>
										...
										<p class="description">
											<?php
 printf( __( 'Send %s a link to reset their password. This will not change their password, nor will it force a change.' ), esc_html( $profile_user->display_name ) ); ?>
										</p>
									</td>
								</tr>
							<?php endif; ?>
					...
					<?php if ( wp_is_application_passwords_available_for_user( $user_id ) || ! wp_is_application_passwords_supported() ) : ?>
						<div class="application-passwords hide-if-no-js" id="application-passwords-section">
							...
							<?php if ( wp_is_application_passwords_available_for_user( $user_id ) ) : ?>
								<?php
 if ( is_multisite() ) : $blogs = get_blogs_of_user( $user_id, true ); $blogs_count = count( $blogs ); if ( $blogs_count > 1 ) : ?>
										<p>
											<?php
 $message = _n( 'Application passwords grant access to <a href="%1$s">the %2$s site in this installation that you have permissions on</a>.', 'Application passwords grant access to <a href="%1$s">all %2$s sites in this installation that you have permissions on</a>.', $blogs_count ); if ( is_super_admin( $user_id ) ) { $message = _n( 'Application passwords grant access to <a href="%1$s">the %2$s site on the network as you have Super Admin rights</a>.', 'Application passwords grant access to <a href="%1$s">all %2$s sites on the network as you have Super Admin rights</a>.', $blogs_count ); } printf( $message, admin_url( 'my-sites.php' ), number_format_i18n( $blogs_count ) ); ?>
										</p>
										<?php
 endif; endif; ?>
 ...
 do_action( 'wp_create_application_password_form', $profile_user ); ?>
									...
									<?php
 else : wp_admin_notice( __( 'Your website appears to use Basic Authentication, which is not currently compatible with Application Passwords.' ), array( 'type' => 'error', 'additional_classes' => array( 'inline' ), ) ); endif; ?>

								<div class="application-passwords-list-table-wrapper">
									<?php
 $application_passwords_list_table = _get_list_table( 'WP_Application_Passwords_List_Table', array( 'screen' => 'application-passwords-user' ) ); $application_passwords_list_table->prepare_items(); $application_passwords_list_table->display(); ?>
								</div>
							<?php elseif ( ! wp_is_application_passwords_supported() ) : ?>
								...
								<p>
									<?php
 printf( __( 'If this is a development website you can <a href="%s" target="_blank">set the environment type accordingly</a> to enable application passwords.' ), __( 'https://developer.wordpress.org/apis/wp-config-php/#wp-environment-type' ) ); ?>
								</p>
							<?php endif; ?>
						</div>
					<?php endif; ?>
					...
					<?php
 if ( IS_PROFILE_PAGE ) { do_action( 'show_user_profile', $profile_user ); } else { do_action( 'edit_user_profile', $profile_user ); } ?>
				...
				<?php if ( count( $profile_user->caps ) > count( $profile_user->roles ) && ( true === $display_additional_caps ) ) : ?>
					...
					<table class="form-table" role="presentation">
						<tr class="user-capabilities-wrap">
							...
							<td>
								<?php
 $output = ''; foreach ( $profile_user->caps as $cap => $value ) { if ( ! $wp_roles->is_role( $cap ) ) { if ( '' !== $output ) { $output .= ', '; } if ( $value ) { $output .= $cap; } else { $output .= sprintf( __( 'Denied: %s' ), $cap ); } } } echo $output; ?>
							</td>
						</tr>
					</table>
				<?php endif; ?>
		...
		<div class="notice notice-success is-dismissible new-application-password-notice" role="alert">
			<p class="application-password-display">
				<label for="new-application-password-value">
					<?php
 printf( __( 'Your new password for %s is:' ), '<strong>{{ data.name }}</strong>' ); ?>
				</label>
