<?php
 ...
 printf( __( 'Additional images added to this gallery: %s' ), '{{ data.ids.length - 5 }}' ); ?>
		...
		<?php
 } protected function has_content( $instance ) { if ( ! empty( $instance['ids'] ) ) { $attachments = wp_parse_id_list( $instance['ids'] ); _prime_post_caches( $attachments, false, false ); foreach ( $attachments as $attachment ) { if ( 'attachment' !== get_post_type( $attachment ) ) { return false; } } return true; } return false; } } 